-- =============================================================================
-- ENTREGABLE: Creacion del Esquema Completo — QuindioFlix
-- Archivo  : ENT_01_creacion_esquema.sql
...
-- =============================================================================
-- Este script consolida en orden de ejecucion todo lo que construyen los
-- scripts Flyway V1, V2, V2.1 y los cambios DDL de V4.1:
--
--   1. Tablespaces (V1)
--   2. Tablas con restricciones y comentarios (V2)
--   3. Columnas adicionales y particion 2026 (V2.1)
--   4. Ajustes DDL finales: rename ciudad, CHECKs de PAGOS (V4.1)
--
-- PREREQUISITO: Conectado como SYSTEM o usuario con privilegios DBA.
--   El usuario de aplicacion QUINDIOFLIX debe existir previamente con:
--     CREATE USER QUINDIOFLIX IDENTIFIED BY quindioflix2026;
--     GRANT CONNECT, RESOURCE, UNLIMITED TABLESPACE TO QUINDIOFLIX;
--
-- EJECUCION: Run as Script (F5) en SQL Developer.
--   Si las tablas ya existen en la BD, los bloques de DROP previenen
--   errores — ejecutar como QUINDIOFLIX una vez creados los tablespaces.
-- =============================================================================

-- Nota sobre rutas de tablespace: ajustar la ruta segun la maquina local.
-- En Oracle XE la ruta tipica es C:\oraclexe\oradata\XE\
-- o el directorio de datos configurado en la instalacion.
...
-- ts_quindioflix_datos: almacena todas las tablas del dominio principal
CREATE TABLESPACE ts_quindioflix_datos
  DATAFILE 'ts_quindioflix_datos01.dbf'
    SIZE 100M
...
-- ts_quindioflix_indices: almacena todos los indices PK y UNIQUE
-- Separar indices de datos reduce la contension de I/O en consultas frecuentes
CREATE TABLESPACE ts_quindioflix_indices
  DATAFILE 'ts_quindioflix_indices01.dbf'
    SIZE 50M
...
-- REPRODUCCIONES es la tabla de mayor volumen (una fila por sesion de usuario)
-- Particionar por anio permite purgar historicos sin afectar la particion activa
CREATE TABLESPACE ts_quindioflix_reprod_2024
  DATAFILE 'ts_quindioflix_reprod_202401.dbf'
    SIZE 200M
...
-- ts_quindioflix_reprod_2025: particiones p_2025 y p_2026 de REPRODUCCIONES
CREATE TABLESPACE ts_quindioflix_reprod_2025
  DATAFILE 'ts_quindioflix_reprod_202501.dbf'
    SIZE 200M
    AUTOEXTEND ON NEXT 100M MAXSIZE 2G
  EXTENT MANAGEMENT LOCAL AUTOALLOCATE
  SEGMENT SPACE MANAGEMENT AUTO
...
-- =============================================================================
-- PASO 2: TABLAS CON RESTRICCIONES Y COMENTARIOS
-- Orden: respeta dependencias FK (tablas padre antes que hijas)
--   PLANES -> USUARIOS -> PERFILES -> PAGOS, REPRODUCCIONES, etc.
--   DEPARTAMENTOS <-> EMPLEADOS (FK ciclica resuelta con ALTER al final)
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- 1. PLANES
--    Tres planes de suscripcion: Basico ($14.900), Estandar ($24.900),
--    Premium ($34.900). Determina precio, calidad, pantallas y max perfiles.
-- -----------------------------------------------------------------------------
CREATE TABLE PLANES (
...
-- -----------------------------------------------------------------------------
-- 2. DEPARTAMENTOS
--    Cinco departamentos de la empresa. FK hacia EMPLEADOS (id_jefe) es
--    ciclica y se agrega al final, despues de crear EMPLEADOS.
-- -----------------------------------------------------------------------------
CREATE TABLE DEPARTAMENTOS (
                               id_departamento   NUMBER(3)       GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                               nombre            VARCHAR2(50)    NOT NULL,
                               id_jefe           NUMBER(6)       DEFAULT NULL,
...
COMMENT ON COLUMN DEPARTAMENTOS.id_jefe         IS 'FK hacia EMPLEADOS: empleado que dirige este departamento.';


-- -----------------------------------------------------------------------------
-- 3. EMPLEADOS
--    Personal de QuindioFlix con jerarquia de supervision interna.
--    Relacion reflexiva id_supervisor: NULL si no tiene supervisor.
-- -----------------------------------------------------------------------------
CREATE TABLE EMPLEADOS (
                           ...
                           id_supervisor     NUMBER(6)       DEFAULT NULL,
...
COMMENT ON COLUMN EMPLEADOS.id_supervisor   IS 'FK reflexiva hacia EMPLEADOS: supervisor directo. NULL si no tiene supervisor.';
...
-- -----------------------------------------------------------------------------
-- 4. CATEGORIAS
--    Clasificacion del contenido: Peliculas, Series, Documentales, Musica, Podcasts.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. GENEROS
--    Generos del catalogo. Un contenido puede tener varios generos (N:M).
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN GENEROS.nombre    IS 'Nombre del genero: Accion, Comedia, Drama, Suspenso, etc.';


-- -----------------------------------------------------------------------------
-- 6. CONTENIDO
--    Entidad central: peliculas, series, documentales, musica y podcasts.
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN CONTENIDO.duracion_min              IS 'Duracion en minutos. NULL para series y podcasts (duracion en episodios).';
COMMENT ON COLUMN CONTENIDO.clasificacion_edad        IS 'Clasificacion de edad: TP, +7, +13, +16 o +18.';
COMMENT ON COLUMN CONTENIDO.es_original_quindioflix   IS 'S si es produccion propia; N si es contenido licenciado.';
...
COMMENT ON COLUMN CONTENIDO.estado                    IS 'Estado: ACTIVO (disponible), INACTIVO (retirado), PROXIMAMENTE (anunciado).';
COMMENT ON COLUMN CONTENIDO.id_categoria              IS 'FK hacia CATEGORIAS.';
COMMENT ON COLUMN CONTENIDO.id_empleado_publicacion   IS 'FK hacia EMPLEADOS: responsable de la publicacion.';


-- -----------------------------------------------------------------------------
-- 7. CONTENIDO_GENEROS — Tabla intermedia N:M entre CONTENIDO y GENEROS
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 8. CONTENIDO_RELACIONADO — Relaciones entre contenidos (secuelas, remakes...)
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE  CONTENIDO_RELACIONADO               IS 'Relaciones entre contenidos: secuelas, precuelas, remakes, spin-offs y adaptaciones.';
COMMENT ON COLUMN CONTENIDO_RELACIONADO.id_origen     IS 'FK hacia CONTENIDO: contenido de origen.';
COMMENT ON COLUMN CONTENIDO_RELACIONADO.id_destino    IS 'FK hacia CONTENIDO: contenido de destino.';
COMMENT ON COLUMN CONTENIDO_RELACIONADO.tipo_relacion IS 'Tipo de relacion: SECUELA, PRECUELA, REMAKE, SPIN-OFF o ADAPTACION.';


-- -----------------------------------------------------------------------------
-- 9. TEMPORADAS — Organiza series y podcasts en temporadas numeradas
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN TEMPORADAS.anio_estreno      IS 'Año de estreno. Puede ser NULL.';
COMMENT ON COLUMN TEMPORADAS.id_contenido      IS 'FK hacia CONTENIDO.';


-- -----------------------------------------------------------------------------
-- 10. EPISODIOS — Episodios de cada temporada
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN EPISODIOS.id_temporada    IS 'FK hacia TEMPORADAS.';


-- -----------------------------------------------------------------------------
-- 11. USUARIOS
--     Cuentas registradas en la plataforma. ciudad_residencia es el nombre
--     final de la columna (renombrada desde 'ciudad' en V4.1).
--     telefono y fecha_nacimiento agregados en V2.1.
-- -----------------------------------------------------------------------------
CREATE TABLE USUARIOS (
                          ...
                          ciudad_residencia   VARCHAR2(80)    DEFAULT NULL,
                          telefono            VARCHAR2(20)    DEFAULT NULL,
                          fecha_nacimiento    DATE            DEFAULT NULL,
                          id_plan             NUMBER(3)       NOT NULL,
                          id_referidor        NUMBER(8)       DEFAULT NULL,
...
COMMENT ON COLUMN USUARIOS.es_moderador       IS 'S si el usuario tiene rol de moderador; N en caso contrario.';
...
-- -----------------------------------------------------------------------------
-- 12. PERFILES — Perfiles por cuenta. El maximo depende del plan.
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN PERFILES.tipo_perfil IS 'ADULTO: acceso completo. INFANTIL: acceso restringido a TP, +7 y +13.';
COMMENT ON COLUMN PERFILES.avatar      IS 'URL o ruta del avatar. Puede ser NULL.';
COMMENT ON COLUMN PERFILES.id_usuario  IS 'FK hacia USUARIOS: cuenta propietaria de este perfil.';


-- -----------------------------------------------------------------------------
-- 13. PAGOS
--     Registro de cada cobro mensual. Los CHECKs de metodo_pago y estado_pago
--     reflejan los valores corregidos en V4.1.
-- -----------------------------------------------------------------------------
CREATE TABLE PAGOS (
                       ...
                       CONSTRAINT chk_pagos_metodo     CHECK (metodo_pago IN ('TARJETA_CREDITO', 'TARJETA_DEBITO', 'PSE', 'EFECTIVO', 'NEQUI', 'DAVIPLATA')),
                       CONSTRAINT chk_pagos_estado     CHECK (estado_pago IN ('PENDIENTE', 'EXITOSO', 'FALLIDO', 'REEMBOLSADO')),
...
-- -----------------------------------------------------------------------------
-- 14. REPRODUCCIONES
--     Tabla de mayor volumen. Particionada por RANGE sobre fecha_hora_inicio.
--     Particiones: p_2024, p_2025, p_2026
-- -----------------------------------------------------------------------------
...
COMMENT ON TABLE  REPRODUCCIONES                  IS 'Registro de cada sesion de reproduccion. Particionada por anio sobre fecha_hora_inicio.';
COMMENT ON COLUMN REPRODUCCIONES.id_reproduccion  IS 'Identificador unico de la reproduccion. Parte de la PK compuesta.';
COMMENT ON COLUMN REPRODUCCIONES.fecha_hora_inicio IS 'Fecha y hora de inicio. Define la particion (2024, 2025 o 2026).';
COMMENT ON COLUMN REPRODUCCIONES.fecha_hora_fin    IS 'Fecha y hora de fin. NULL si la sesion esta activa.';
COMMENT ON COLUMN REPRODUCCIONES.dispositivo       IS 'Dispositivo: CELULAR, TABLET, TV o COMPUTADOR.';
COMMENT ON COLUMN REPRODUCCIONES.porcentaje_avance IS 'Porcentaje reproducido (0 a 100).';
COMMENT ON COLUMN REPRODUCCIONES.id_perfil         IS 'FK hacia PERFILES.';
COMMENT ON COLUMN REPRODUCCIONES.id_contenido      IS 'FK hacia CONTENIDO.';
COMMENT ON COLUMN REPRODUCCIONES.id_episodio       IS 'FK hacia EPISODIOS. NULL si el contenido no tiene episodios.';
...
COMMENT ON TABLE  CALIFICACIONES                    IS 'Calificaciones de contenido realizadas por los perfiles.';
COMMENT ON COLUMN CALIFICACIONES.id_calificacion    IS 'Identificador unico de la calificacion.';
COMMENT ON COLUMN CALIFICACIONES.estrellas          IS 'Puntuacion: entre 1 y 5 estrellas.';
COMMENT ON COLUMN CALIFICACIONES.resena             IS 'Resena textual opcional.';
COMMENT ON COLUMN CALIFICACIONES.fecha_calificacion IS 'Fecha en que se registro la calificacion.';
COMMENT ON COLUMN CALIFICACIONES.id_perfil          IS 'FK hacia PERFILES.';
COMMENT ON COLUMN CALIFICACIONES.id_contenido       IS 'FK hacia CONTENIDO.';


-- -----------------------------------------------------------------------------
-- 16. FAVORITOS — Lista personal de contenido favorito por perfil
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN FAVORITOS.id_perfil      IS 'FK hacia PERFILES.';
COMMENT ON COLUMN FAVORITOS.id_contenido   IS 'FK hacia CONTENIDO.';


-- -----------------------------------------------------------------------------
-- 17. REPORTES_INAPROPIADO
--     Reportes de contenido inadecuado atendidos por moderadores.
-- -----------------------------------------------------------------------------
...
COMMENT ON COLUMN REPORTES_INAPROPIADO.motivo           IS 'Descripcion del motivo del reporte.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.estado_reporte   IS 'Estado: PENDIENTE, EN_REVISION, RESUELTO o DESCARTADO.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.fecha_reporte    IS 'Fecha en que se realizo el reporte.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.fecha_resolucion IS 'Fecha de resolucion. NULL si aun esta pendiente.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.id_perfil_reporta IS 'FK hacia PERFILES: perfil que reporto.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.id_contenido     IS 'FK hacia CONTENIDO: contenido reportado.';
COMMENT ON COLUMN REPORTES_INAPROPIADO.id_moderador     IS 'FK hacia USUARIOS: moderador asignado. NULL hasta asignacion.';
...
WHERE  table_name IN (
                      ...
                      'REPRODUCCIONES','CALIFICACIONES','FAVORITOS','REPORTES_INAPROPIADO'
    )
ORDER  BY table_name;
...
PROMPT === Esquema QuindioFlix creado exitosamente — 17 tablas ===
-- =============================================================================
-- FIN DEL SCRIPT ENT_01_creacion_esquema.sql
-- Tablas creadas: 17
-- Tablespaces: ts_quindioflix_datos, ts_quindioflix_indices,
--              ts_quindioflix_reprod_2024, ts_quindioflix_reprod_2025
-- Particiones REPRODUCCIONES: p_2024, p_2025, p_2026
-- =============================================================================